...
// filter prob better
  const i = bookList.findIndex(b => b.title === removeTitle && b.author === removeAuthor);
  if (i !== -1) bookList.splice(i, 1);
  return bookList
};

// (3) Write an arrow function that prints all book titles using .forEach()
const printBooks = () => {
  bookList.forEach(b => console.log(b.title));
};
...
switch(day){
  ...
  case "Friday": console.log("Last day!"); break;
  case "Saturday": console.log("Weekend!"); break;
  case "Sunday": console.log("Rest day."); break;
...
const temperature = (celsius) => {
  ...
  if (f >= 80) return `wear shorts`;
  if (f >= 60) return `light jacket`;
  return `bundle up`;
};
